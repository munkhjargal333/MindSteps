...
import { 
  ...
  Star, Globe, X, UploadCloud, AlertCircle, ChevronRight
} from 'lucide-react';
...
export default function NewLessonPage() {
  ...
  const [formData, setFormData] = useState({
    title: '',
    slug: '',
    parentCategoryId: 0, // ШИНЭ: Parent категори сонгох
    categoryId: 0,       // Эцсийн child категори
    description: '',
    content: '',
  ...
  // Parent category сонгоход child категориудыг авах
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium mt-0.5">Контент удирдлага</p>
            </div>
          </div>
          <button
...
- Жагсаалт 2"
        ...
        <div className="space-y-6">
          ...
          {/* Category Selection - ШИНЭЧЛЭГДСЭН */}
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
            <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
              ...
              <select 
                ...
                onChange={(e) => {
                  ...
                  setFormData({
                    ...
                    categoryId: 0 // Reset child when parent changes
                  });
                }}
              >
            ...
            {/* Child Category - Зөвхөн parent сонгогдсон үед харагдана */}
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div>
              ...
              <label className={`flex items-center gap-4 p-4 rounded-2xl border-2 border-dashed transition-all cursor-pointer ${
                ...
                <div className={`p-3 rounded-xl shadow-sm ${files.media ? 'bg-emerald-500' : 'bg-white'}`}>
                  {formData.lessonType === 'video' ? (
                    <Video size={20} className={files.media ? 'text-white' : 'text-gray-400'} />
                  ) : (
                    <Music size={20} className={files.media ? 'text-white' : 'text-gray-400'} />
                  )}
                </div>
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-bold text-gray-900 truncate">
                    {files.media ? files.media.name : 'Файл оруулах'}
                  </p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {files.media ? 'Амжилттай' : 'MP4, MOV, MP3 хүртэл 100MB'}
                  </p>
                </div>
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-2 block">Хугацаа (мин)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="text-xs font-bold text-gray-700 mb-2 block">Оноо</label>
                  <input 
                    type="number" 
                    className="w-full bg-gray-50 border-2 border-gray-100 focus:border-indigo-500 focus:ring-0 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-900 transition-all" 
              ...
              <div>
                <label className="text-xs font-bold text-gray-700 mb-2 block">Шошго (Tags)</label>
                <div className="relative">
                  <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                  <input 
                    placeholder="stress, health, wellness" 
                    className="w-full bg-gray-50 border-2 border-gray-100 focus:border-indigo-500 focus:ring-0 rounded-xl pl-11 pr-4 py-2.5 text-sm font-medium text-gray-900 transition-all" 
                    value={formData.tags} 
                    onChange={(e) => setFormData({...formData, tags: e.target.value})} 
                  />
                </div>
                <p className="text-xs text-gray-400 mt-1.5 ml-1">Таслалаар тусгаарлана</p>
              </div>
            </div>
          </section>
        </div>
